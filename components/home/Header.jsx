...
export default function Header({ session }) {
  ...
  const loginRedirection = session ? "/dashboard/today" : "/login"
  const loginText = session ? <p>Mon dashboard</p> : <p>Login</p>

  return (
    <header className="w-full h-[80px] p-[20px_32px]">
      <nav className="relative flex justify-between items-center max-w-[432px] screen-480:max-w-[512px] screen-640:max-w-none tablet:max-w-[720px] screen-960:max-w-[1024px] screen-1200:max-w-[1440px] mx-auto">
        ...
        <div className="max-screen-960:hidden flex justify-between items-center gap-[6px] font-bold screen-1024:text-lg">
          <Link href={loginRedirection} className="p-[8px_20px] rounded-lg text-[var(--text-color)] hover:text-[var(--primary-color)] transition-all ease duration-150">{loginText}</Link>
        ...
        <div className="screen-960:hidden text-2xl flex justify-center items-center">
          <button className="cursor-pointer" onClick={() => setShowNavbar(!showNavbar)}>
            <AiOutlineMenu />
          </button>
        </div>
        {
          showNavbar ? (
            <HamburgerMenu setShowNavbar={setShowNavbar} showNavbar={showNavbar} loginText={loginText} loginRedirection={loginRedirection}/>
          ) : (null)
        }
      </nav>
    </header>
